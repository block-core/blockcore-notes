...
import { Event, nip05, nip98 } from 'nostr-tools';
...
export class FilesComponent {
  ...
  constructor(
    ...
    effect(async () => {
      if (this.appState.connectedSignal()) {
        await this.load();
      }
    });
  }
  ...
  //     return await nip98.sign(payload, method, url);
  //   } else {
  //     throw new Error('Missing payload, method or url in the event tags.');
  //   }
  // }
  ...
  async load() {
    this.loading = true;
    const listUrl = 'https://nostrcheck.me/api/v2/media?page=0&count=100';

    const authToken = await nip98.getToken(listUrl, 'get', async(e) => {
      return await this.sign(e);
    }, true)

    const page0 = await fetch(listUrl, {
      method: 'GET',
      headers: {
    ...
    if (page0.ok) {
      const json = await page0.json();
      console.log(json);
    }

    const uploadUrl = 'https://nostrcheck.me/api/v2/media';
    const uploadType: 'media' | 'avatar' | 'banner' = 'media';
